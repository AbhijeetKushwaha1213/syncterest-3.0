...
-- Phase 5: Integrate Privacy Settings into Advanced Search

-- Step 1: Update the advanced search function to respect user privacy settings.
-- This modification ensures that:
-- 1. Only profiles marked as 'public' are included in search results.
-- 2. Distance filtering and sorting only apply to users who have enabled location sharing.
    ...
    where_clause := '
        WHERE
            ...
            AND (
                ...
                ' || quote_nullable(p_latitude) || ' IS NULL OR ' || quote_nullable(p_longitude) || ' IS NULL OR ' || quote_nullable(p_radius_km) || ' IS NULL OR p.show_location IS NOT TRUE OR p.latitude IS NULL OR p.longitude IS NULL OR
                (6371 * acos(
    ...
    IF p_sort_by = 'distance' THEN
        query := 'SELECT * FROM public.profiles p ' || where_clause || '
                  ORDER BY (CASE
                                ...
                                WHEN ' || quote_nullable(p_latitude) || ' IS NOT NULL AND ' || quote_nullable(p_longitude) || ' IS NOT NULL AND p.show_location IS TRUE AND p.latitude IS NOT NULL AND p.longitude IS NOT NULL
                                THEN (6371 * acos(
                                        cos(radians(' || p_latitude || ')) * cos(radians(p.latitude)) *
                                        cos(radians(p.longitude) - radians(' || p_longitude || ')) +
                                        sin(radians(' || p_latitude || ')) * sin(radians(p.latitude))
