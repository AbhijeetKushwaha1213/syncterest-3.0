...
const EnhancedPeopleNearYou = () => {
  const { profile } = useAuth();
  const { profileLocation } = useLocation();
  const [hoveredProfile, setHoveredProfile] = useState<string | null>(null);
  const { followMutation } = useFollow();

  // Extract latitude and longitude from profileLocation (as you wanted to implement)
  ...
  const { data: profiles, isLoading } = useQuery<Profile[]>({
    queryKey: ["profiles_nearby_enhanced", profile?.id, latitude, longitude],
    queryFn: async () => {
      let queryProfiles: Profile[] = [];
      
      // Use the extracted latitude and longitude
      if (profile && latitude && longitude) {
        console.log('Searching for nearby users with coordinates:', { latitude, longitude });
        ...
        if (error) {
          ...
          console.log(`Found ${queryProfiles.length} nearby users`);
        }
      }

      // Fallback to random profiles if not enough nearby users
      if (queryProfiles.length < 5) {
        ...
        if(randomData) {
          queryProfiles.push(...randomData);
          console.log(`Added ${randomData.length} random users to fill the list`);
        }
      }
  ...
  const getSubtext = (p: Profile) => {
    // Use the extracted latitude and longitude for comparison
    if (p.latitude && p.longitude && latitude && longitude) {
        return "Nearby 📍";
    }
...
export default EnhancedPeopleNearYou;